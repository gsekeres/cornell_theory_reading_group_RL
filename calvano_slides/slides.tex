...
\begin{frame}\frametitle{An Issue}
	If you initialize $Q_0$ precisely the way that they say they do, stuff breaks immediately. Specifically, we saw one of two patterns: either for all parameters convergence was to the same pair ($\approx 1.58$), which is not the collusive equilibrium, or convergence was to a clearly suboptimal pair (\emph{e.g.} $1.8926$ and $1.4277$). 
	
	This was fixed entirely by removing $(1-\delta)$ from the denominator.
	\[
...
\begin{frame}\frametitle{Why does this happen?}
	...
	\end{itemize}
\end{frame}
\begin{frame}\frametitle{Some Theories}
	\begin{itemize}
		\item The obvious note is that this is the discounted payoff until infinity
		\item The issue is that this is not how learning works. We incorporate $\delta$ by updating one period at a time, not sending time far into the future: \[Q(s,a) = \mathbb{E}(\pi \mid s,a) + \textcolor{red}{\delta} \mathbb{E}\left[ \max_{a'\in A} Q(s',a') \mid s,a\right]\]
		\item By including the tail, we massively overestimate the value to each action
		\item $\varepsilon$ degenerating quickly explains convergence to a random suboptimal pair
	\end{itemize}
\end{frame}
\begin{frame}\frametitle{Some More Theories}
	\begin{itemize}[<+->]
		\item Why did we get to that $(1.58,1.58)$ pair, however? \bigskip
		\item {\huge \shrug} \bigskip
		\item We'd love any theories you have. Finn's: 
		\item[]\begin{center}
			\alert{One of the algo just beat the other one to death and force them to play Nash}
		\end{center}
	\end{itemize}
\end{frame}
