...
# Load results for 2 agents model

numAgents = 2
numStates = numPrices^numAgents
det = fread(file = "A_det_1_2agents.txt", 
            header = TRUE,
            verbose = TRUE)
...
con = file("InfoExperiment_1_2agents.txt", open = "r")
...
save(det, 
     ...
     file = "det_2agents.Rdata")

# Load results for 3 agents model

numAgents = 3
numStates = numPrices^numAgents
det = fread(file = "A_det_1_3agents.txt", 
            header = TRUE,
            verbose = TRUE)
det = det[order(det$Session, 
                det$DevToPrice, 
...
Strategies = matrix(data = NA, nrow = numStates*numAgents, numGames)
strategy = matrix(data = NA, nrow = numStates, ncol = numAgents)
con = file("InfoExperiment_1_3agents.txt", open = "r")
for (iGame in 1:numGames) {
    if ((iGame %% 100) == 0) cat("iGame = ", iGame, "\n")
    q = readLines(con, n = 1) # Skip iGame line    
    Converged[iGame] = as.numeric(readLines(con, n = 1))    
...
save(det, 
     Strategies, Converged, TimeToConvergence,
     CycleLength, CycleStates, CyclePrices, CycleProfits, maxCycleLength, 
     file = "det_3agents.Rdata")

# Load results for 4 agents model

numAgents = 4
numStates = numPrices^numAgents
det = fread(file = "A_det_1_4agents.txt", 
            header = TRUE,
            verbose = TRUE)
det = det[order(det$Session, 
                det$DevToPrice, 
...
Strategies = matrix(data = NA, nrow = numStates*numAgents, numGames)
strategy = matrix(data = NA, nrow = numStates, ncol = numAgents)
con = file("InfoExperiment_1_4agents.txt", open = "r")
for (iGame in 1:numGames) {
    if ((iGame %% 100) == 0) cat("iGame = ", iGame, "\n")
    q = readLines(con, n = 1) # Skip iGame line    
    Converged[iGame] = as.numeric(readLines(con, n = 1))    
